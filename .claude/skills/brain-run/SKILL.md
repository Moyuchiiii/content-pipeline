...
- **売れる構成 絶対ルール遵守（後述 A〜F）**: 目次必須・冒頭 4 点セット（ペイン→実績→ベネフィット→目次）・無料ゾーン末尾フックの 3 大要素は Phase 5 の構成適合性スウィープで必ず検証される。違反したら自動 rewrite
...
- **冒頭に番号付き目次を必ず配置する**（売れてる 8 商品全てに目次あり。目次なし商品はゼロ）
- Brain の商品ページは H2 見出しを自動的にサイドバー目次として表示する。本文内にも「目次」節を置いて番号付きリストで全章を提示する
- 目次を省略すると「何が書いてあるかわからない商品」として購入判断できず離脱する
...
4. **目次（番号付き）** — 全章のナビゲーション

**NG 順序（過去の失敗事例）**: 「この記事で手に入るもの（ベネフィットリスト）」から始めて、ペインを出さず、実績数字も薄く、目次もない順序。これは売れない。
...
7. やらないこと
8. 同梱物の全体像
9. 教科書の全体像（Day1〜Day{N} カリキュラム）
10. 著者プロフィール
11. Day1: {具体的な章タイトル}
12. Day2: {具体的な章タイトル}
...
{N+10}. ここから先でやること一覧（無料ゾーン末尾のフック）
...
1. 価格煽り（先着○部限定 or ○部ごとに値上げ）※任意
2. こんな悩みありませんか？（ペイン）
3. 僕も同じ場所にいた（共感）
4. ターニングポイント（気づきの瞬間）
5. 現在の実績
6. この記事で手に入ること
7. このBrainの中身（目次のチラ見せ）
8. モニター感想
9. おすすめな人 / おすすめしない人
10. 価格・特典
11. FAQ
...
{第 1 章〜最終章の目次をチラ見せ形式で。各章 1〜2 行の概要付き}
...
**【目次・構造】**
1. [ ] 冒頭に番号付き目次がある（本文内に `## 目次` 節 + 番号付きリスト）
2. [ ] 章タイトルが具体的（「第 1 章: 基礎」でなく「なぜ○○は 9 割失敗するのか」「Day1: 環境構築で最初の感動体験」形式）
3. [ ] 章数は 6〜18 章
...
**スウィープ実行方法:**
記事全文を読み、各項目を自動判定してチェックマークを付ける。欠落項目がある場合はその項目に対応するテンプレ（タイプ A/B/C）の該当セクションを参照して記事に追記・再配置する。特に「ベネフィットから始まっている（ペインなし）」「目次なし」「フックなし」の 3 大 NG は最優先で修正する。
...
**構成適合性（20 点満点）:**
- 冒頭 4 点セット（ペイン→実績→ベネフィット→目次）の有無・順序（0-5）
- 目次の具体性・章タイトルの型（Why 型 / Day 型 / 問いかけ型等）（0-5）
