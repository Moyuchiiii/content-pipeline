...
- **売れる構成 絶対ルール遵守（後述 A〜F）**: 見出し具体化（Brain 自動目次前提）・冒頭 3 点セット（ペイン→実績→ベネフィット）・無料ゾーン末尾フックの 3 大要素は Phase 5 の構成適合性スウィープで必ず検証される。違反したら自動 rewrite
...
使用可能なHTMLタグ（note エディタと同じ想定で運用可）:
- `<h2>` `<h3>` — 見出し（`<h1>` はタイトルフィールドに使うため本文では使わない）
- `<p>` — 段落
- `<b>` — 強調
- `<blockquote>` — 引用（ラベル付きは `<b>ラベル</b><br>本文` 形式・コロン禁止）
- `<pre><code>` — コードブロック
- `<ul><li>` / `<ol><li>` — 箇条書き・番号付き
- `<hr>` — 区切り線
- `<a href="...">` — リンク
- `<table><tr><td>` — 表
- `<code>` — インラインコード

使用可能なMarkdown記法（バックアップ用・Brain に直接は貼らない）:
- `# ` `## ` `### ` — 見出し
- `**太字**` — 強調
- `> ` — 引用
- `` `コード` `` — インラインコード
...
<!-- 注意: H2 見出しに章番号（「## 1.」「## 2.」等）を付けない。Brain サイドバーが H2 から自動で目次を生成するため、章番号は二重化する。本文内にも「## 目次」節を置かない -->
...
### 著者の実績
{文系大学生が 2 ヶ月で月 14 万、Claude Code × CW で副業 ○件受注等、具体数字}
...
| Day | タイトル | 所要時間 | ゴール |
|---|---|---|---|
| Day1 | {具体的な章タイトル} | ○分 | {到達目標} |
| Day2 | {具体的な章タイトル} | ○分 | {到達目標} |
...

## 著者プロフィール

{文系大学生 × Claude Code × CW × 2 ヶ月月 14 万}
...
{第 1 章〜最終章の章タイトル一覧を箇条書きでチラ見せ。各章 1〜2 行の概要付き。※これは Brain 自動目次とは別で、有料ゾーンの「中身」を販促として見せるパート}

## モニター感想

{発売前のモニター 2〜3 名の感想を引用。発売前で未取得なら「モニター募集中・感想は発売後追記」と明記}
...
**判定:**
- 13 項目以上 OK → 公開可
- 10〜12 項目 OK → 軽微修正で公開可（欠落項目のみピンポイント rewrite）
- 9 項目以下 OK → 自動 rewrite 実行（Phase 2 の該当タイプテンプレ A/B/C を参照しながら全体を再構成。最大 2 回、2 回後も基準未達ならユーザー報告）

**スウィープ実行方法:**
記事全文を読み、各項目を自動判定してチェックマークを付ける。欠落項目がある場合はその項目に対応するテンプレ（タイプ A/B/C）の該当セクションを参照して記事に追記・再配置する。特に「ベネフィットから始まっている（ペインなし）」「H2 に章番号が付いている」「フックなし」の 3 大 NG は最優先で修正する。
...
- Step 2 の 15 項目チェックリスト通過率（13 項目以上 = 5 点 / 10〜12 項目 = 3 点 / 9 項目以下 = 0 点）
