...
| 引数 | 処理 |
|---|---|
| （なし） | **全系統スキャン**（A/B/C/D/E 5 系統並行）+ ネタ帳投入 + **Phase 6 今日のプラン生成（plan.json 書き出し）** |
| `plan` | **スキャンせず、既存ネタ帳の現状から today/plan.json だけ生成・更新する軽量モード（毎回必須・1分）** |
| `note` | note向けスキャン（A 副業 + B Claude新機能）→ noteネタ帳投入 + plan.json更新 |
| `x` | X向けスキャン（B Claude新機能 + D 公式アカウント）→ Xネタ帳投入 + plan.json更新 |
| `brain` | Brain向けスキャン（C Brain市場 + 過去note資産）→ Brainネタ帳投入 + plan.json更新 |
| `light` | 曜日分散の軽量スキャン（旧ロジック）+ plan.json更新 |
| `dry-run` | 投入せず結果表示のみ（テスト用・plan.json も書き出さない） |
| `add` | ユーザー手動投入モード（対話形式で1ネタ追加） |

**plan サブコマンド（2026-04-30 新設・運用必須）:**

content-pipeline を使う日は**必ず最初に `/source-run plan` を叩いて today/plan.json を更新する**。各 run（note-run / brain-run / x-run）は起動時に plan.json を読み込んで「今日の自分の役割」を確認する設計。plan.json なし or 当日付でない場合、各 run は警告を出してフォールバック動作になる。

「今日もう plan あるけど、新ネタ拾いたい」場合は通常の `/source-run`（フル実行・スキャン込み）を再実行すれば plan.json も最新化される。
...
## Phase 6: 今日のコンテンツプラン生成（today/plan.json 書き出し・2026-04-30 新設）

各 run（note-run / brain-run / x-run）が「今日の自分の役割」を共有するための **Single Source of Truth** ファイルを生成する。

### 6.1. 生成タイミング

- 引数なし `/source-run`（フル実行）→ Phase 1〜5 完了後に Phase 6 実行
- `/source-run plan`（軽量）→ Phase 1〜5 をスキップして Phase 6 のみ実行
- `/source-run note|x|brain|light` → 該当系統のスキャン後に Phase 6 実行
- `/source-run dry-run` → Phase 6 はスキップ（plan.json は書き出さない）

### 6.2. plan.json スキーマ

書き出し先: `projects/content-pipeline/today/plan.json`

```json
{
  "schema_version": 1,
  "date": "2026-04-30",
  "generated_at": "2026-04-30T09:15:00+09:00",
  "generated_by": "source-run",
  "regenerated_count": 0,

  "note": {
    "execute": true,
    "primary_topic": {
      "notion_page_id": "3528795a-75fd-816f-...",
      "title": "ネタ名そのまま",
      "type": "速報 | 実録 | ノウハウ",
      "priority": "高 | 中 | 低",
      "reason": "なぜこれを選んだか1行"
  ...
  "brain": {
    "execute": false,
    "skip_reason": "なぜ今日Brainやらないか1行（または null）",
    "primary_topic": null,
    "alternatives": [
      { "notion_page_id": "...", "title": "..." }
    ]
  ...
  "x": {
    "execute": true,
    "slots": {
      "morning":  { "type": "実況",     "topic_id": "<notion_page_id|null>", "title": "...", "推奨時間帯": "朝07:00" },
      "noon":     { "type": "速報所感", "topic_id": "<notion_page_id|null>", "title": "...", "推奨時間帯": "昼12:30" },
      "night1":   { "type": "告知",     "topic_id": "pending_cta:auto",       "title": "今日のnote記事告知", "推奨時間帯": "夜20:00" },
      "night2":   { "type": "引用RT",   "topic_id": "<notion_page_id|null>", "title": "...", "推奨時間帯": "夜23:00" }
    },
    "expected_pending_cta_from": ["note-run", "brain-run"]
  },

  "execution_order": ["note-run", "x-run"],

  "actual": {
    "note_executed": null,
    "note_topic_id_used": null,
    "brain_executed": null,
    "brain_topic_id_used": null,
    "x_executed": null,
    "x_slots_used": null
  }
}
...
`actual` セクションは各 run が実行後に書き戻すフィールド（PDCA素材）。source-run 生成時は全部 null。

### 6.3. 生成ロジック

#### note セクション

**CRITICAL — 記事タイプ分散ハードフロア（2026-09-02 追加・note-run側フロアがバイパスされていた不具合の修正）:**

note-run/SKILL.md Phase 2 に同種のハードフロアがあるが、`plan.primary_topic` が存在すると note-run はスコアリングごとスキップして即採用する設計（Phase 1 Step 0）のため、**source-run がここで速報を選ぶと note-run 側のフロアは素通りされる**。2026-05-01〜06-14 の速報9連投は、旧ロジックの「第3優先: 速報 > 実録 > ノウハウ」が同点時に速報を選び続けたことが根本原因だった[実機確認済 published-history.md・note-performance.md]。primary_topic 選定の**前**に以下を適用する:

1. `context/published-history.md`「note 投稿済み」直近5件の記事タイプ内訳を確認
2. 速報が3本以上 → 候補から `記事タイプ候補=速報` を除外して以下の並び替えに進む
3. 5本とも速報 → さらにノウハウも除外し実録候補のみに絞る（実録が最転換率のため）
4. 絞り込みで候補が0件になった場合のみ、除外前の全候補に戻す（速報ゼロ状態を避けるためのフォールバック）

1. noteネタ帳（`collection://1a603b4f-d1e4-4ed7-8c75-c7c0a5b7e595`）の `ステータス=未使用` レコード全件取得（上記フロアで絞り込み済みの場合はその候補内で）
2. 並び替え:
   - **第1優先: 優先度=高**
   - **第2優先: 発見日が新しい**
   - **第3優先: 実録 > ノウハウ > 速報**（2026-09-02 note-strategy.md「記事タイプ優先順位」と整合するよう逆転。ハードフロアで速報が残っている場合のみ効く同点条件）
3. トップ1件を `primary_topic` に設定
4. 次点2〜3件を `alternatives` に設定
5. `execute` 判定:
   - `primary_topic` あり かつ 優先度=高 OR 中 → `execute: true`
   - 全候補が優先度=低 → `execute: false`、`skip_reason: "高優先ネタなし"`
   - ネタ帳空 → `execute: false`、`skip_reason: "noteネタ帳に未使用ネタなし"`
...
1. Xネタ帳（`collection://6af7d0ce-64a0-4347-a5e3-325d27b440b8`）の `ステータス=未使用` レコード取得
2. 時間スロット別に振り分け:
   - **morning（朝07:00）**: `推奨時間帯=朝` の優先度=高/中から1本
   - **noon（昼12:30）**: `推奨時間帯=昼` の優先度=高 or `ネタタイプ=速報所感` の最新ネタ
   - **night1（夜20:00）**: 
     - 第1優先: `note.execute=true` または `brain.execute=true` なら → `topic_id: "pending_cta:auto"` で告知枠予約
     - 第2優先: 曜日が水/日 → `ネタタイプ=ノウハウ` でスレッド枠
     - 第3優先: `推奨時間帯=夜前半` から1本
   - **night2（夜23:00）**: `ネタタイプ=引用RT候補 OR 日常 OR 問いかけ` から1本
3. 該当ネタなしのスロットは `null` で残す（x-run で柔軟対応）
4. `execute` 判定:
   - スロットいずれか1個でも埋まっていれば `execute: true`
   - 全スロット null なら `execute: false`、`skip_reason: "Xネタ帳に未使用ネタなし・告知もなし"`

### 6.4. execution_order の決定

```
note.execute=true かつ x.execute=true       → ["note-run", "x-run"]
note.execute=true かつ brain.execute=true   → ["note-run", "brain-run", "x-run"]
brain.execute=true のみ                     → ["brain-run", "x-run"]
x.execute=true のみ                         → ["x-run"]
全部 false                                  → []（実行不要）
```

### 6.5. 既存 plan.json がある場合の挙動

- 同日付 plan.json が既存 → `regenerated_count` を +1 して上書き
- 異なる日付 → そのまま上書き（前日分は明示的にアーカイブしない・必要なら content-pipeline 側で git 履歴から復元）
- `actual` フィールドが既に書き込まれていた場合 → 上書き前にバックアップ（`today/plan-backup-{timestamp}.json`）

### 6.6. Phase 5 報告への組み込み

完了報告の最後に「📋 今日のコンテンツプラン」セクションを追加:

```
📋 今日のコンテンツプラン（plan.json 生成済 ✅）

【note】 execute: ✅
  Primary: {title} ({priority}・{type})
  理由: {reason}
  代替: {alternatives count}件

【brain】 execute: ❌（{skip_reason}）

【X】 execute: ✅
  朝07:00  {type}: {title}
  昼12:30  {type}: {title}
  夜20:00  告知: ↑のnote記事を pending_cta 経由で取込
  夜23:00  {type}: {title}

実行順: {execution_order を矢印付きで}

→ /note-run を最初に実行してください
```

---

## モード: plan（軽量・スキャンなし・2026-04-30 新設）

`/source-run plan` で起動。**Phase 1〜5（広域スキャン・ネタ帳投入）をスキップ**して、既存ネタ帳の現状からだけで plan.json を生成・更新する軽量モード。

### 用途

- 朝起きて「今日の作業計画立てたい」が、ネタ帳に十分な未使用ネタがある時
- フル `/source-run` を昨日叩いて、今日新ネタ仕込みは不要な時
- 計画だけ更新したい時

### 実行フロー

1. **Phase 0**: 必要ファイル読み込み（published-history.md / persona-hyui.md のみ・他はスキップ可）
2. **Phase 6**: plan.json 生成（上記 6.3 ロジック）
3. **完了報告**: Phase 5 のフル報告ではなく、軽量版の plan サマリのみ出力

### 完了報告例（plan モード）

```
✅ /source-run plan 完了（軽量モード・スキャンなし）

📋 今日のコンテンツプラン

【note】 execute: ✅
  Primary: Sundar Pichai Gemini Docs/Sheets/Slides 直接生成
  理由: 高優先・速報24h以内・109万閲覧バズ
  代替: 2件（Sydney+Canva/Xero / Anthropic売上$30B）

【brain】 execute: ❌（Brain直近投稿2日前・販売動向観察期間）

【X】 execute: ✅（4スロット）
  朝07:00  Before/After: /recap 復帰時間20分→1分
  昼12:30  速報所感: Sundar Pichai所感
  夜20:00  告知: ↑のnote記事を pending_cta 経由で取込
  夜23:00  引用RT: ゴブリン風刺

実行順: /note-run → /x-run

ファイル: today/plan.json (regenerated_count: 0)

ネタ帳の未使用在庫:
- noteネタ帳: 未使用 12件（高 4 / 中 6 / 低 2）
- Xネタ帳: 未使用 14件（高 3 / 中 8 / 低 3）
- Brainネタ帳: 未使用 2件（high 2）

⚠️ ネタ帳在庫が薄くなってきたら /source-run（フル）でスキャン補充してください
```

### 在庫アラート

各ネタ帳の `ステータス=未使用` 件数が閾値以下なら警告:

| ネタ帳 | 警告閾値 | 推奨アクション |
|---|---|---|
| noteネタ帳 | 5件未満 | `/source-run note` で補充 |
| Xネタ帳 | 8件未満 | `/source-run x` で補充 |
| Brainネタ帳 | 1件未満 | `/source-run brain` で補充 |
