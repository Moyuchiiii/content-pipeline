...
### 🚨 絶対ルール（2026-04-24 追加・厳守）

**5 系統（A / B / C / D / E）すべてを実行する。1 系統でも省略禁止。**

- 「もう十分なネタが見つかった」と途中で判断して中断するのは禁止
- 各系統で 0 件ヒットでも、エラー記録を残して次へ進む
- Chrome MCP がログアウト状態（Phase 0 で検知済み）の場合のみ系統 D・E スキップ可。それ以外は必ず実行
- WebSearch のクエリは**指定された全クエリ**を実行（数を勝手に減らさない）
...
1. **5系統のうち実行されたのが3つ未満** → ⚠️ 警告: 「Phase 1 を完全実行できていません。系統 X, Y がスキップされた理由を確認してください」
2. **Xネタ帳投入 0 件 + 系統 D・E のいずれかが ❌** → ⚠️ 警告: 「Chrome MCP のログイン状態を確認してください」
3. **全系統合計投入数が 3 件未満** → ⚠️ 警告: 「スキャン結果が極端に少ない。クエリの妥当性を見直してください」
