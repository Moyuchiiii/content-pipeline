...
2. `today/note/` の最新記事（あればBefore/Afterや失敗談を抜き出し）
...
[WebSearch済 comnico.jp/we-love-social/x-algorithm + akagami.blog/sns2026x] 2026年X アルゴリズムは **「投稿直後30分のエンゲージメント」が最重要シグナル**。投稿予約しただけで放置するとアルゴリズム評価が伸びない。

#### 投稿時刻における運用必須事項

1. **投稿時刻にスマホ・PCを開く**: Typefully から投稿された直後30分以内に X を必ず開く
2. **リプライが来たら即返信**: リプ間隔が短いほどアルゴリズムボーナス最大化
3. **本文URL貼らない**: 既存ルール通り・URL はリプライ専用
4. **「いいね」ボタンを自分で押さない**: 不自然な伸び方は逆にマイナス評価

#### 推奨: 投稿時刻リマインダー

このスキルから直接通知は出さないが、ユーザー側で以下を設定:
- iPhone/Android 通知を Typefully アプリで投稿時刻 ON
- または投稿時刻 30 分前にスマホアラーム

#### ストーリー型ツイート週2本ノルマ（2026-05-07 追加）

[WebFetch済 akagami.blog/sns2026x] X アルゴリズム分析: **ストーリー型（失敗→気づき→成長）がエンゲージメント総量最高**。

- `/source-run` の Xネタ帳に **「ストーリー型」ネタタイプ** を新設（2026-05-07・Notion DB 拡張済み）
- 月水金の night1 枠などに意図的に組み込む
- hyui の「1〜2歩先を歩く先輩」フレームと最も相性の良い投稿タイプ

#### plan.x.slots へのストーリー型枠マッピング（2026-05-07 追加）

`/source-run plan` 起動時、以下の曜日条件で自動的にストーリー型枠を埋める:

| 曜日 | night1 (20:00) ストーリー型枠 |
|---|---|
| 月 | ✅ ストーリー型を優先採用（Xネタ帳「未使用 + ネタタイプ=ストーリー型」） |
| 水 | ✅ ストーリー型 or ノウハウスレッド枠（既存仕様） |
| 金 | ✅ ストーリー型を優先採用 |
| 火・木 | 通常運用（告知 or 引用RT） |
| 土・日 | 通常運用（雑談 or 引用RT） |

Xネタ帳に「ストーリー型」未使用ネタが0件なら、その日は通常運用にフォールバック（強制しない）。

詳細: `context/note-strategy.md`「フォロワー段階別 戦略マトリクス」/ `search/2026-05-07/sns-marketing-research.md`
