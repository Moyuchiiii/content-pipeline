...
```
content-pipeline/
  today/                        # 最新投稿用（常にここを開く・2026-04-28 統合改修）
    note/                       # /note-run の出力先（draft/delivery/サムネ）
                                # 次回 /note-run 実行時に自動で note/drafts/ へ移動
    brain/                      # /brain-run の出力先（html/md/meta/images_md）
      images/                   # /brain-run 一括生成画像
        {記事ID}/               # サムネ + 本文画像（thumb.jpg + 1.jpg〜N.jpg）
                                # 次回 /brain-run 実行時に自動で brain/drafts/ へ移動
    source-run/                 # /source-run の失敗バックアップ等（failed-{date}.json）
  context/                      # 共有コンテキスト
  ...
  brain/
    ...
    published/                  # 投稿済みログ
    # ※ 2026-04-28 から brain/today/ は撤廃 → today/brain/ に統合
  .claude/skills/
